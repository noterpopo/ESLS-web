              ...
              <div class="editorarea" v-if="reRenderFlag" :style="{ zIndex:'9999',border:'1px solid black', width:styleWidth+'px',height:styleHeight+'px'}">
                <Poptip v-for="(item,index) in currentDispmsData" :key="index" trigger="click" title="编辑框" class="poptipWarp" :style="{ position: 'absolute',left: item.x+'px',top: item.y+'px'}">
                    <vue-draggable-resizable :id="'poptip'+index"  v-show="item.status===1" :style="{fontSize:'14px', backgroundColor:item.backgroundColor==='0'?'black':item.backgroundColor==='1'?'white':'red',position: 'relative',left: 0+'px',top: 0+'px'}" :x="item.x" :y="item.y" :w="item.width" :h="item.height" class-name-active="draggerItem-active-class" class-name="draggerItem-class" @activated="onActivated(index)" @dragging="onDrag(arguments,index)" @resizing="onResize(arguments,index)" parent=".editorarea">
                        ...
                        <div :style="{textAlign:'left',position: 'relative',top:'-'+parseInt(parseInt(item.height)-3)+'px',height:item.backup+'px',background : '#000',overflow:'hidden',border:'0px'}" v-else-if="item.columnType === '线段'&&item.width>item.height"></div>
